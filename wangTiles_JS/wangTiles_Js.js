...
function draw() {
  ...
  for (var y = 0; y < nbRow; y++) {
    for (var x = 0; x < nbCol; x++) {
      ...
      var posY = y * 32 + 16;
      var id = x + y * nbCol;
      //   if (myWalker.tiles[id].weight>-255)myWalker.tiles[id].weight--;
      image(tileSet[valueTiles[id]], posX, posY);
      if (isDrawingText)
        text(myTiles[id].nb, posX, posY);
    }
  }
  myWalker.update();
  myWalker.display();
  fill(255);
  if (frameCount < 200)
    text("press or touch for generate another map", width / 2, height / 2);
  // drawGrid();
...
function generateMaze() {
  var myMaze = createMaze(nbCol, nbRow);
  myTiles.length = 0;
 valueTiles = myMaze.getMaze();
  for (var y = 0; y < nbRow; y++) {
    for (var x = 0; x < nbCol; x++) {
      var nb = x + y * nbCol;
      var id = valueTiles[nb];
      var tTile = new Tile();
      tTile.create(tileSet[id], id, nb, x, y);
      myTiles.push(tTile);
    }
  }
}
...
function weightTile(tileA, tileB) {
  if (tileA.weight < tileB.weight) {
    return -1;
  } else if (tileA.weight > tileB.weight) {
    return 1;
  } else
    return 0;
}
